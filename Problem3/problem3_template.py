...
class MyTradingFunctions():
	...
	def __init__(self):  #Put any global variables here
		self.lookback = 120  ## TODO: max number of historical datapoints you want at any given time
		self.dataSetId = 'qq5p3data'
		self.params = {}
	...
	##############################################################################
		ask_mom1Dict = {'featureKey': 'ask_mom_5',
				   'featureId': 'momentum',
				   'params': {'period': 5,
							  'featureName': 'askPrice'}}
		ask_mom2Dict = {'featureKey': 'ask_mom_10',
				   'featureId': 'momentum',
				   'params': {'period': 10,
							  'featureName': 'askPrice'}}
		ask_ma1Dict = {'featureKey': 'ask_ma_2',
		...
		bid_mom1Dict = {'featureKey': 'bid_mom_5',
					   'featureId': 'momentum',
					   'params': {'period': 5,
							  'featureName': 'bidPrice'}}
		bid_mom2Dict = {'featureKey': 'bid_mom_10',
				   'featureId': 'momentum',
				   'params': {'period': 10,
							  'featureName': 'bidPrice'}}
		bid_ma1Dict = {'featureKey': 'bid_ma_2',
				   'featureId': 'moving_average',
				   'params': {'period': 2,
							  'featureName': 'bidPrice'}}
	...
	def predictFairPrice(self, time, updateNum, instrumentManager, marketSeries):
		...
		#############################################################################################
		###  TODO 3 : FILL THIS FUNCTION TO RETURN A fairprice for each stockID                   ###
		###  You can use all the features created above and combine then using any logic you like ###
		###  USE TEMPLATE BELOW AS EXAMPLE                                                        ###
		#############################################################################################

		# First we get the fairprice, bid and ask price series for all stocks
		fairPrice = lookbackInstrumentFeatures.getFeatureDf('fairPrice')
		askPrice = lookbackInstrumentFeatures.getFeatureDf('askPrice')
		bidPrice = lookbackInstrumentFeatures.getFeatureDf('bidPrice')

		#This gets us our position in all stocks
		position = lookbackInstrumentFeatures.getFeatureDf('position')

		for stockId in self.getSymbolsToTrade():
			# if we want the fairPrice for a single stock
			fairPriceStock = fairPrice[stockId]

			# This is a way to access the feature df's for every stock
			ask_ma_5 = lookbackInstrumentFeatures.getFeatureDf('ask_ma_2')[stockId]
			bid_ma_5 = lookbackInstrumentFeatures.getFeatureDf('bid_ma_2')[stockId]

			# In this simple case, we are just setting our next fairPrice to be the previous fairprice.
			marketSeries[stockId] = fairPriceStock.iloc[-1]
		return marketSeries
